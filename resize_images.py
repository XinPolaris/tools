...
def main():
    ...
    if not files:
        print("未找到符合扩展名的图片文件。扩展名列表:", sorted(exts))
        sys.exit(0)
    ...
    for i, f in enumerate(files, 1):
        # determine relative path to preserve folder structure when recursive
        rel = os.path.relpath(f, start=input_dir)
        out_file = os.path.join(output_root, rel)
        # ensure extension remains same; if inplace, out_file == f
        if args.inplace:
            out_file = f
        else:
            # ensure parent dir exists (will be created in process)
            pass
